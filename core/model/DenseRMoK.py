...
class DenseRMoK(nn.Module):
    ...
    def forward(self, var_x, marker_x):
        ...
        # print(f"var_x shape after slicing: {var_x.shape}") 
        # # Before RevIN normalization
        # if self.training and not hasattr(self, 'printed'):
        #     var_x_mean_before = var_x.mean().item()
        #     var_x_std_before = var_x.std().item()
        #     print(f"Before RevIN normalization - var_x mean: {var_x_mean_before:.4f}, std: {var_x_std_before:.4f}")


        var_x = self.rev(var_x, 'norm') if self.rev else var_x

        # # After RevIN normalization
        # if self.training and not hasattr(self, 'printed'):
        #     var_x_mean_after = var_x.mean().item()
        #     var_x_std_after = var_x.std().item()
        #     print(f"After RevIN normalization - var_x mean: {var_x_mean_after:.4f}, std: {var_x_std_after:.4f}")
        ...
        #  #After RevIN denormalization
        # if self.training and not hasattr(self, 'printed'):
        #     prediction_mean = prediction.mean().item()
        #     prediction_std = prediction.std().item()
        #     print(f"After RevIN denormalization - prediction mean: {prediction_mean:.4f}, std: {prediction_std:.4f}")
